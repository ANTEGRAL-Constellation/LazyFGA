...
describe("matchRule", () => {
  ...
  test("fan-out rule only matches when the array attribute is non-empty", () => {
    ...
    const fr = writeRule({ fanOut: "roleKeys", tupleTemplate: { user: "user:{{subject}}", relation: "{{item}}", object: "team:{{attributes.project}}" } });
...
describe("renderTuple (injection guard)", () => {
  test("substitutes placeholders into a valid tuple (subject + subject.id alias)", () => {
    const e2: IdpEvent = { type: "x", subject: { type: "user", id: "alice" }, attributes: { project: "123" } };
    ...
    expect(
      renderTuple({ user: "user:{{subject.id}}", relation: "member", object: "team:{{attributes.project}}" }, e2).tuple,
  ...
  test("rejects forbidden chars in substituted value (no userset/wildcard injection)", () => {
    const e2: IdpEvent = { type: "x", subject: { type: "user", id: "alice" }, attributes: { project: "1#member" } };
  ...
  test("array attribute in a scalar slot errors (must use fan-out)", () => {
    const e2: IdpEvent = { type: "x", subject: { type: "user", id: "alice" }, attributes: { project: ["a", "b"] } };
    expect(renderTuple(writeRule().tupleTemplate, e2).error).toMatch(/array|fan-out/);
  });

  test("{{item}} without a provided item errors", () => {
    const e2: IdpEvent = { type: "x", subject: { type: "user", id: "alice" }, attributes: { project: "1" } };
    expect(
      renderTuple({ user: "user:{{subject}}", relation: "{{item}}", object: "team:{{attributes.project}}" }, e2).error,
    ).toMatch(/item/);
  });
  ...
  test("{{item}} binds the fan-out element", () => {
    const e2: IdpEvent = { type: "x", subject: { type: "user", id: "alice" }, attributes: { project: "1" } };
    expect(
      renderTuple({ user: "user:{{subject}}", relation: "{{item}}", object: "team:{{attributes.project}}" }, e2, "editor").tuple,
    ).toEqual({ user: "user:alice", relation: "editor", object: "team:1" });
  });

  test("requires literal type: prefix (event must not choose the type)", () => {
    const e2: IdpEvent = { type: "x", subject: { type: "user", id: "alice" }, attributes: { t: "document", project: "123" } };
    expect(
      renderTuple({ user: "user:{{subject}}", relation: "member", object: "{{attributes.project}}" }, e2).error,
    ).toMatch(/literal type/);
    expect(
      renderTuple({ user: "user:{{subject}}", relation: "member", object: "{{attributes.t}}:{{attributes.project}}" }, e2).error,
    ).toMatch(/literal type/);
  });
});
...
describe("applyEvents", () => {
  test("applies matched rules; counts applied", async () => {
    ...
    expect(await applyEvents([ev], [writeRule()], deps)).toEqual({ applied: 1, skipped: 0, failed: 0 });
  });

  test("no matching rule → skipped", async () => {
    const e2: IdpEvent = { type: "unmatched", subject: { type: "user", id: "alice" }, attributes: {} };
    const { deps } = fakeDeps(async () => "applied");
  ...
  test("idempotent write (already exists) → skipped, not failed", async () => {
    ...
    expect(await applyEvents([ev], [writeRule()], deps)).toEqual({ applied: 0, skipped: 1, failed: 0 });
  });

  test("render error → failed, continues", async () => {
    const e2: IdpEvent = { type: "user.grant.added", subject: { type: "user", id: "alice" }, attributes: {} };
    const { deps } = fakeDeps(async () => "applied");
  ...
  test("deterministic write error → failed, continues", async () => {
    ...
    expect(await applyEvents([ev], [writeRule()], deps)).toEqual({ applied: 0, skipped: 0, failed: 1 });
  });
  ...
  describe("array fan-out", () => {
    const fanRule = (): MappingRule =>
      writeRule({
        fanOut: "roleKeys",
        tupleTemplate: { user: "user:{{subject}}", relation: "{{item}}", object: "team:{{attributes.project}}" },
      });

    test("emits one tuple per array element", async () => {
      const e2: IdpEvent = { type: "user.grant.added", subject: { type: "user", id: "alice" }, attributes: { project: "1", roleKeys: ["editor", "viewer"] } };
      const tuples: RenderedTuple[] = [];
      const { deps } = fakeDeps(async (_op, t) => {
        tuples.push(t);
        return "applied";
      });
      expect(await applyEvents([e2], [fanRule()], deps)).toEqual({ applied: 2, skipped: 0, failed: 0 });
    ...
    test("a forbidden-char element fails only that element; the rest apply", async () => {
      const e2: IdpEvent = { type: "user.grant.added", subject: { type: "user", id: "alice" }, attributes: { project: "1", roleKeys: ["editor", "bad role"] } };
      const { deps } = fakeDeps(async () => "applied");
      expect(await applyEvents([e2], [fanRule()], deps)).toEqual({ applied: 1, skipped: 0, failed: 1 });
    });

    test("empty array → rule does not match → skipped (no tuples)", async () => {
      const e2: IdpEvent = { type: "user.grant.added", subject: { type: "user", id: "alice" }, attributes: { project: "1", roleKeys: [] } };
      const { deps } = fakeDeps(async () => "applied");
      expect(await applyEvents([e2], [fanRule()], deps)).toEqual({ applied: 0, skipped: 1, failed: 0 });
    });
  });
});
