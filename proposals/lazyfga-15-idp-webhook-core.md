...
- CONCEPT 차별: *"Bring your IdP."* 인증은 IdP, 인가는 lazyFGA. 어느 IdP든 공통분모는 **"OIDC claims + webhook"** 하나다(§5).
...
export interface IdpEvent { type: string; subject: { id: string }; attributes: Record<string, string>; }
...
export function applyEvents(events: IdpEvent[], rules: MappingRule[]): Promise<{ applied: number; skipped: number; failed: number }>;
```
