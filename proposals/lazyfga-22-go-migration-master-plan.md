...
6. **Framework default bodies are matched, not deviated**: unknown routes return Hono's default `404 Not Found` (text/plain) and unhandled infrastructure errors return Hono's default `Internal Server Error` (text/plain), reproduced exactly in the Go router (including Hono strict-mode trailing-slash 404 and percent-decoded path params) so the LFGA-27 contract replay sees no framework-default diffs.
