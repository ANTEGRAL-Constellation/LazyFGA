...
2. **Malformed UUID path params → 404** (LFGA-25/26): TS forwards malformed UUIDs to Postgres and surfaces a 500; Go validates and returns 404 for `/model/versions/:id`, `/tokens/:id`, `/idp/connections/:id`, `/idp/rules/:ruleId` (and the rules-of-connection route). Strictly-better error mapping; the web never emits malformed ids.
3. **Audit `from`/`to` accepted date formats** (LFGA-26): TS accepts anything JS `new Date(...)` parses; Go accepts RFC3339 (with or without fractional seconds) and `YYYY-MM-DD` (UTC midnight), returning the same 400 message otherwise. Docs state the accepted formats.
...
6. **Framework default bodies are matched, not deviated**: unknown routes return Hono's default `404 Not Found` (text/plain) and unhandled infrastructure errors return Hono's default `Internal Server Error` (text/plain), reproduced exactly in the Go router so the LFGA-27 contract replay sees no framework-default diffs.
